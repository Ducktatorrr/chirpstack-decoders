...
function Decode(fPort, bytes, variables) {
	...
	var cc = ((bytes[1 + i] << 16) | (bytes[2 + i] << 8) | bytes[3 + i]).toString(
		10
	);
	var dd = ((bytes[4 + i] << 16) | (bytes[5 + i] << 8) | bytes[6 + i]).toString(
		10
	);
	var ee = getMyDate(
		(
...
function Decode(fPort, bytes, variables) {
	if (fPort == 0x03) {
		...
		if (bytes.length == 11 || bytes.length == 18) {
			return {
				Node_type: "LDS03A",
				ALARM1: alarm1,
